//! Bard module — Ideas content CRUD courtier.
//!
//! Handles the `idea.*` namespace: create, list, load, update_title,
//! save, delete, search. Each handler composes Ideas + Vault + Hall
//! across crate boundaries in pure Rust (no FFI).
//!
//! Uses `prerogative::api_json` for all outbound serialization (x::Value
//! unwrapping, header flattening, field renaming). Inbound save uses
//! load-merge-write: loads the existing package, applies TS changes,
//! writes back via Hall.
...
use prerogative::api_json;

pub struct BardModule;
...
fn guard_vault_unlocked(state: &DaemonState) -> Result<(), PhoneError> {
    ...
    if !vault.is_unlocked() {
        return Err(err("bard", "Vault is locked — unlock identity first"));
    }
    Ok(())
}

/// Lightweight Polity pre-check — blocks operations that require
/// explicit user consent via UI dialog. Content CRUD is allowed
/// for the identity owner acting on their own data.
fn polity_check(op: &str) -> Result<(), PhoneError> {
    let always_ask = [
        "chamberlain.delete",
        "chamberlain.import",
        "castellan.wipe",
...
impl DaemonModule for BardModule {
    fn id(&self) -> &str { "bard" }
    fn name(&self) -> &str { "Bard (Ideas Content)" }
    fn deps(&self) -> &[&str] { &["chamberlain", "castellan"] }

    fn register(&self, state: &Arc<DaemonState>) {
        // ── idea.create ─────────────────────────────────────────
        let s = state.clone();
        state.phone.register_raw("bard.create", move |data| {
            polity_check("bard.create")?;
            ...
            // Optional: join an existing shared note by specifying its UUID.
            // If the note already exists locally, return it instead of creating a duplicate.
            let explicit_id = params.get("id").and_then(|v| v.as_str())
                .and_then(|s| Uuid::parse_str(s).ok());
            if let Some(eid) = explicit_id {
                let vault = s.vault.lock().unwrap();
                if let Ok(Some(existing)) = vault.get_idea(&eid) {
                    let root_id: Option<Uuid> = existing.header_cache.as_ref()
                        .and_then(|json_str| serde_json::from_str::<Value>(json_str).ok())
                        .and_then(|v| v.get("root_digit_id").and_then(|r| r.as_str()).and_then(|s| Uuid::parse_str(s).ok()));
                    return ok_json(&api_json::manifest_entry_json(&existing, root_id.as_ref()));
                }
            }
            ...
            let author = omnibus.pubkey_hex()
                .ok_or_else(|| err("bard.create", "No identity — create one first"))?;
            ...
            // Override with explicit UUID (for joining shared notes)
            if let Some(eid) = explicit_id {
                header.id = eid;
            }
            ...
            let personal = vault.personal_path().map_err(|e| err("bard.create", e))?;
            ...
            s.email.send_raw("bard.created", &result_bytes);
        ...
        // ── idea.list ───────────────────────────────────────────
        let s = state.clone();
        state.phone.register_raw("bard.list", move |data| {
            guard_vault_unlocked(&s)?;
        ...
        // ── idea.load ───────────────────────────────────────────
        let s = state.clone();
        state.phone.register_raw("bard.load", move |data| {
            guard_vault_unlocked(&s)?;

            let params: Value = serde_json::from_slice(data).unwrap_or(Value::Null);
            let id_str = params.get("id").and_then(|v| v.as_str())
                .ok_or_else(|| err("bard.load", "missing 'id'"))?;
            let id = Uuid::parse_str(id_str)
                .map_err(|e| err("bard.load", format!("invalid UUID: {e}")))?;

            let mut vault = s.vault.lock().unwrap();
            let entry = vault.get_idea(&id)
                .map_err(|e| err("bard.load", e))?
                .ok_or_else(|| err("bard.load", "idea not found"))?;
        ...
        // ── idea.update_title ───────────────────────────────────
        // Lightweight title update: patches the root digit's title property
        // and the manifest entry without a full package merge.
        let s = state.clone();
        state.phone.register_raw("bard.update_title", move |data| {
            guard_vault_unlocked(&s)?;

            let params: Value = serde_json::from_slice(data).unwrap_or(Value::Null);
            let id_str = params.get("id").and_then(|v| v.as_str())
                .ok_or_else(|| err("bard.update_title", "missing 'id'"))?;
            let id = Uuid::parse_str(id_str)
                .map_err(|e| err("bard.update_title", format!("invalid UUID: {e}")))?;
            let title = params.get("title").and_then(|v| v.as_str())
                .ok_or_else(|| err("bard.update_title", "missing 'title'"))?;

            let mut vault = s.vault.lock().unwrap();
            let entry = vault.get_idea(&id)
                .map_err(|e| err("bard.update_title", e))?
                .ok_or_else(|| err("bard.update_title", "idea not found"))?
                .clone();
            ...
            // Patch root digit title property
            let root_id = package.header.content.root_digit_id;
            if let Some(root) = package.digits.get_mut(&root_id) {
                root.properties.insert("title".to_string(), x::Value::String(title.to_string()));
                root.modified = chrono::Utc::now();
            }
            ...
            updated_entry.title = Some(title.to_string());
            ...
            s.email.send_raw("bard.saved", &event);
        ...
        let s = state.clone();
        state.phone.register_raw("bard.save", move |data| {
            polity_check("bard.save")?;
            guard_vault_unlocked(&s)?;

            let params: Value = serde_json::from_slice(data)
                .map_err(|e| err("bard.save", e))?;
            let id_str = params.get("id").and_then(|v| v.as_str())
                .ok_or_else(|| err("bard.save", "missing 'id'"))?;
            let id = Uuid::parse_str(id_str)
                .map_err(|e| err("bard.save", format!("invalid UUID: {e}")))?;

            let incoming_pkg = params.get("package")
                .ok_or_else(|| err("bard.save", "missing 'package'"))?;
            ...
            let entry = vault.get_idea(&id)
                .map_err(|e| err("bard.save", e))?
                .ok_or_else(|| err("bard.save", "idea not found"))?
                .clone();
            ...
            if let Some(digits_obj) = incoming_pkg.get("digits").and_then(|d| d.as_object()) {
                ...
                for (digit_id_str, digit_json) in digits_obj {
                    let digit_id = Uuid::parse_str(digit_id_str)
                        .map_err(|e| err("bard.save", format!("invalid digit UUID: {e}")))?;
                    ...
                    if let Some(existing) = package.digits.get_mut(&digit_id) {
                        ...
                        let new_digit: ideas::Digit = serde_json::from_value(digit_serde)
                            .map_err(|e| err("bard.save", format!("failed to create digit: {e}")))?;
        ...
        // ── idea.delete ─────────────────────────────────────────
        let s = state.clone();
        state.phone.register_raw("bard.delete", move |data| {
            polity_check("bard.delete")?;
            guard_vault_unlocked(&s)?;

            let params: Value = serde_json::from_slice(data).unwrap_or(Value::Null);
            let id_str = params.get("id").and_then(|v| v.as_str())
                .ok_or_else(|| err("bard.delete", "missing 'id'"))?;
            let id = Uuid::parse_str(id_str)
                .map_err(|e| err("bard.delete", format!("invalid UUID: {e}")))?;
            ...
            vault.unregister_idea(&id)
                .map_err(|e| err("bard.delete", e))?;

            // Post-modifier: emit event
            let event = serde_json::to_vec(&json!({"id": id_str})).unwrap_or_default();
            s.email.send_raw("bard.deleted", &event);
        ...
        // ── idea.search ─────────────────────────────────────────
        let s = state.clone();
        state.phone.register_raw("bard.search", move |data| {
            guard_vault_unlocked(&s)?;
            ...
            let hits = vault.search(query, limit)
                .map_err(|e| err("bard.search", e))?;

            let hits_json: Vec<Value> = hits.iter()
                .map(api_json::search_hit_json)
                .collect();
    ...
    fn catalog(&self) -> ModuleCatalog {
        ModuleCatalog::new()
            .with_call(CallDescriptor::new("bard.create", "Create a new .idea"))
            .with_call(CallDescriptor::new("bard.list", "List ideas (filtered)"))
            .with_call(CallDescriptor::new("bard.load", "Load an .idea package"))
            .with_call(CallDescriptor::new("bard.update_title", "Update idea title"))
            .with_call(CallDescriptor::new("bard.save", "Save an .idea package"))
            .with_call(CallDescriptor::new("bard.delete", "Delete an .idea"))
            .with_call(CallDescriptor::new("bard.search", "Full-text search"))
            .with_emitted_event(EventDescriptor::new("bard.created", "Idea was created"))
            .with_emitted_event(EventDescriptor::new("bard.saved", "Idea was saved"))
            .with_emitted_event(EventDescriptor::new("bard.deleted", "Idea was deleted"))
    }
}
