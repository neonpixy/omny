...
/// (Program vs Throne vs CLI) happens in the IPC server before dispatch.
...
fn wire_yoke_observers(email: &Email) {
    ...
    for event_id in ["bard.created", "bard.saved", "bard.deleted"] {
...
fn wire_pager_observers(state: &Arc<DaemonState>) {
    ...
    let events = [
        ("chamberlain.created", "Identity created", "chamberlain"),
        ("chamberlain.locked", "Identity locked", "chamberlain"),
        ("chamberlain.unlocked", "Identity unlocked", "chamberlain"),
        ("castellan.locked", "Vault locked", "castellan"),
        ("castellan.unlocked", "Vault unlocked", "castellan"),
    ];
