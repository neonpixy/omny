//! Platform autostart configuration for chancellor.
//!
//! - **macOS**: launchd plist at `~/Library/LaunchAgents/co.omnidea.chancellor.plist`
//! - **Linux**: systemd user unit at `~/.config/systemd/user/chancellor.service`
...
/// Find the chancellor binary path.
...
#[cfg(target_os = "macos")]
const LAUNCHD_LABEL: &str = "co.omnidea.chancellor";
...
#[cfg(target_os = "linux")]
const SYSTEMD_SERVICE_NAME: &str = "chancellor.service";
...
fn install_systemd(daemon_bin: &Path) -> Result<PathBuf, String> {
    ...
    let unit = format!(
        r#"[Unit]
Description=Omnidea Chancellor — Omninet node service
...
mod tests {
    ...
    fn launchd_plist_path_is_valid() {
        ...
        assert!(path.ends_with("co.omnidea.chancellor.plist"));
    ...
    fn systemd_unit_path_is_valid() {
        ...
        assert!(path.ends_with("chancellor.service"));
