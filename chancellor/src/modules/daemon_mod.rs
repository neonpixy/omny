...
/// Get the device's LAN IPv4 address (same technique as Globe's UPnP module).
    ...
    fn register(&self, state: &Arc<DaemonState>) {
        ...
        state.phone.register_raw("daemon.status", move |_data| {
            ...
            let mut result = json!({
                ...
                "network": {
                    "bind_all": config.as_ref().map(|c| c.omnibus.bind_all).unwrap_or(false),
                    "enable_upnp": config.as_ref().map(|c| c.omnibus.enable_upnp).unwrap_or(false),
                    "local_ip": local_ipv4(),
