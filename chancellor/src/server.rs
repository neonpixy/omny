...
use omnibus::Omnibus;
use chancellor_client::{ClientType, Handshake, HandshakeResponse, PushEvent, Request, Response, RpcError};
...
const RESTRICTED_METHODS: &[&str] = &[
    ...
    "chamberlain.delete", "chamberlain.import",
    "config.set", "config.reload",
    "omnibus.stop", "omnibus.restart",
    "envoy.tower_start", "envoy.tower_stop",
];

fn check_permission(client_type: &ClientType, method: &str) -> Result<(), RpcError> {
    match client_type {
        ClientType::Throne | ClientType::Tray | ClientType::Cli => Ok(()),
        ClientType::Program => {
...
/// Equipment Email events to forward to IPC clients as push events.
const FORWARDED_EMAIL_EVENTS: &[&str] = &[
    "artificer.editor_saved",
    "artificer.editor_changed",
    "bard.created",
    "bard.saved",
    "bard.deleted",
    "vizier.sync_message",
    "vizier.awareness_update",
    "vizier.presence_update",
];
...
fn start_event_forwarder(
    ...
    let mut rx_omnibus = omnibus.subscribe_events();

    // Bridge Equipment Email events into a channel for the forwarder thread.
    let (email_tx, email_rx) = std::sync::mpsc::channel::<PushEvent>();
    for &event_name in FORWARDED_EMAIL_EVENTS {
        let tx = email_tx.clone();
        let name = event_name.to_string();
        state.email.subscribe_raw(event_name, move |data| {
            let data_val = serde_json::from_slice(data).unwrap_or(serde_json::Value::Null);
            let _ = tx.send(PushEvent { event: name.clone(), data: data_val });
        });
    }
    drop(email_tx);
    ...
    thread::Builder::new()
        ...
        .spawn(move || {
            while subscribed.load(Ordering::Relaxed) && !state.shutdown.load(Ordering::Relaxed) {
                ...
                // Check Omnibus events
                match rx_omnibus.try_recv() {
                    Ok(omnibus_event) => {
