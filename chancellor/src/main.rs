...
//!
//! ```text
//! chancellor              # run in foreground
//! chancellor --daemon     # daemonize (double-fork, detach, log to file)
//! chancellor status       # query running daemon status
//! chancellor stop         # stop running daemon
//! chancellor install      # install platform autostart
//! chancellor uninstall    # remove platform autostart
//! ```
...
use chancellor::auth;
use chancellor::config::{self, load_or_create_default, omnidea_home, pidfile_path, to_tower_config};
use chancellor::ffi_ops;
use chancellor::modifiers;
use chancellor::modules;
use chancellor::server::IpcServer;
use chancellor::state;

fn main() {
    // Install rustls crypto provider before any TLS connections (wss://).
...
fn print_usage() {
    println!(
        "chancellor — Omnidea node service\n\
         \n\
         Usage:\n\
         \x20 chancellor              Run in foreground\n\
         \x20 chancellor --daemon     Daemonize (background, log to ~/.omnidea/daemon.log)\n\
         \x20 chancellor status       Query running daemon status\n\
         \x20 chancellor stop         Stop running daemon\n\
         \x20 chancellor install      Install platform autostart\n\
         \x20 chancellor uninstall    Remove platform autostart\n\
         \x20 chancellor --help       Show this message"
    );
}
...
fn cmd_install() {
    match autostart::install() {
        Ok(path) => {
            ...
            println!("chancellor will start automatically on login.");
        }
        Err(e) => {
...
fn cmd_uninstall() {
    match autostart::uninstall() {
        Ok(path) => {
            ...
            println!("chancellor will no longer start on login.");
        }
        Err(e) => {
...
fn cmd_daemonize() {
    ...
    match pid {
        ...
        _child_pid => {
            ...
            println!("chancellor daemonizing (pidfile: {})", pidfile_path().display());
            std::process::exit(0);
        }
    }
    ...
    log::info!("chancellor daemonized (pid {})", std::process::id());
...
fn cmd_daemonize() {
    ...
    eprintln!("Use 'chancellor' (foreground) or your platform's service manager.");
    std::process::exit(1);
}

/// Connect to the running daemon and print its status.
fn cmd_status() {
    match chancellor_client::DaemonClient::connect() {
        ...
        Err(e) => {
            ...
            eprintln!("Is chancellor running?");
            std::process::exit(1);
        }
    }
}
...
fn run_foreground() {
    ...
    log::info!("chancellor starting (foreground)");
    run_foreground_inner();
}
...
fn run_foreground_inner() {
    ...
    // Check for an existing daemon — wait for it to exit if shutting down.
    let pidfile = pidfile_path();
    if pidfile.exists() {
        if let Ok(contents) = std::fs::read_to_string(&pidfile) {
            if let Ok(pid) = contents.trim().parse::<u32>() {
                if is_process_alive(pid) {
                    // Wait up to 10 seconds for the old daemon to exit (restart scenario).
                    log::info!("Waiting for previous daemon (pid {pid}) to exit...");
                    ...
                    if !exited {
                        log::error!("Another daemon is still running (pid {pid})");
                        eprintln!("Another daemon is still running (pid {pid}).");
                        eprintln!("Run 'chancellor stop' first, or delete {}", pidfile.display());
                        std::process::exit(1);
                    }
                    log::info!("Previous daemon exited");
    ...
    if daemon_config.tower.enabled {
        ...
        match Omnibus::start(omnibus_config) {
            Ok(o) => {
                ...
                // Connect to home node (Tower relay) if configured.
                if let Some(ref home) = daemon_config.omnibus.home_node {
                    match o.set_home_node(home) {
                        Ok(()) => log::info!("Connected to home node: {home}"),
                        Err(e) => log::warn!("Failed to connect to home node {home}: {e}"),
                    }
                }
    ...
    let socket_path = chancellor_client::default_socket_path();
    ...
    // Vault unlock deferred until crown.unlock with user's password.
