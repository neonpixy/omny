...
impl TestDaemon {
    ...
    fn connect(&self) -> DaemonClient {
        ...
        let home_auth = chancellor_client::auth_token_path();
...
fn e2e_crown_lifecycle() {
    ...
    // 1. No identity yet
    let state = client.call("chamberlain.state", json!({})).unwrap();
    assert_eq!(state["exists"], false);
    assert_eq!(state["unlocked"], false);

    // 2. Create identity
    let created = client.call("chamberlain.create", json!({"name": "E2E User", "password": "test-pw-123"})).unwrap();
    ...
    // 4. Lock
    let locked = client.call("chamberlain.lock", json!({})).unwrap();
    ...
    let profile_err = client.call("chamberlain.profile", json!({}));
    assert!(profile_err.is_err(), "chamberlain.profile should fail when locked");

    // 6. Unlock
    let unlocked = client.call("chamberlain.unlock", json!({"password": "test-pw-123"})).unwrap();
...
fn e2e_idea_crud() {
    ...
    client.call("chamberlain.create", json!({"name": "CRUD Tester", "password": "test-pw-123"})).unwrap();

    // 1. Create
    let created = client.call("bard.create", json!({
        "title": "E2E Note",
        "type": "text",
    ...
    let list = client.call("bard.list", json!({})).unwrap();
    ...
    let loaded = client.call("bard.load", json!({"id": idea_id})).unwrap();
    assert!(loaded.is_object(), "should return IdeaPackage object");

    // 4. Create a second note
    let created2 = client.call("bard.create", json!({
        "title": "Second Note",
        "type": "text",
    ...
    // 6. Delete the first
    let deleted = client.call("bard.delete", json!({"id": idea_id})).unwrap();
...
fn e2e_idea_requires_identity() {
    ...
    let result = client.call("bard.create", json!({"title": "Fail", "type": "text"}));
...
fn e2e_idea_list_filter() {
    ...
    client.call("chamberlain.create", json!({"name": "Filter Tester", "password": "test-pw-123"})).unwrap();

    // Create notes with different titles
    client.call("bard.create", json!({"title": "Alpha Note", "type": "text"})).unwrap();
    client.call("bard.create", json!({"title": "Beta Note", "type": "text"})).unwrap();
    client.call("bard.create", json!({"title": "Alpha Document", "type": "text"})).unwrap();

    // Filter by title_contains
    let alpha = client.call("bard.list", json!({"title_contains": "Alpha"})).unwrap();
    assert_eq!(alpha.as_array().unwrap().len(), 2);

    let beta = client.call("bard.list", json!({"title_contains": "Beta"})).unwrap();
...
fn e2e_vault_locked_until_identity() {
    ...
    let status = client.call("castellan.status", json!({})).unwrap();
    assert_eq!(status["unlocked"], false);

    // Create identity — vault should unlock
    client.call("chamberlain.create", json!({"name": "Vault Tester", "password": "test-pw-123"})).unwrap();
...
fn e2e_op_list() {
    ...
    assert!(op_names.contains(&"chamberlain.create"));
    assert!(op_names.contains(&"bard.create"));
...
fn e2e_unknown_method() {
    ...
    if let Err(chancellor_client::ClientError::Rpc(rpc_err)) = result {
...
fn e2e_identity_aliases() {
    ...
    let created = client.call("identity.create", json!({"name": "Alias Test", "password": "test-pw-123"})).unwrap();
...
fn e2e_multiple_clients() {
    ...
    client1.call("chamberlain.create", json!({"name": "Multi-Client", "password": "test-pw-123"})).unwrap();

    let state = client2.call("chamberlain.state", json!({})).unwrap();
    assert_eq!(state["exists"], true);
    assert_eq!(state["unlocked"], true);
}
...
fn e2e_full_tome_workflow() {
    ...
    let created = client.call("chamberlain.create", json!({"name": "Tome User", "password": "test-pw-123"})).unwrap();
    let _crown_id = created["crown_id"].as_str().unwrap();

    // Step 3: Create a note (what Tome does when user clicks "New Note")
    let note = client.call("bard.create", json!({
    ...
    let loaded = client.call("bard.load", json!({"id": note_id})).unwrap();
    ...
    let deleted = client.call("bard.delete", json!({"id": note_id})).unwrap();
    assert_eq!(deleted["ok"], true);
