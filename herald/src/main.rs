//! herald -- Omnidea system tray / menu bar app.
//!
//! Shows Omnibus status with an animated spinning pinwheel icon.
//! Communicates with the Chancellor via the `chancellor-client` IPC library.
    ...
    fn handle_menu_event(&mut self, id: &MenuId, event_loop: &ActiveEventLoop) {
        ...
        match id_str {
            ...
            s if s == ID_STOP_AND_QUIT => {
                ...
                // Call daemon.stop directly and synchronously — don't route
                // through the status thread, which races with process exit.
                match chancellor_client::DaemonClient::connect() {
                    Ok(client) => {
                        match client.call("daemon.stop", serde_json::json!({})) {
                            Ok(_) => log::info!("Daemon stop confirmed"),
                            Err(e) => log::error!("daemon.stop failed: {e}"),
                        }
                    }
                    Err(e) => log::error!("Could not connect to daemon for stop: {e}"),
                }
